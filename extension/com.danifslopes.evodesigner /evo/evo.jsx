...
var itemMutationProb = 0.33;
var percItemsToMutate = 1.1;
...
function setup() {
    ...
    colorMode(RGB);
    app.activeDocument.pasteboardPreferences.pasteboardMargins = [(width * 4) + 'pt', (height + 4) + 'pt'];
...
//some elements are being deleted. cannot happen. just hide and only non-mandatory!!!! TODO
...
function mutate(pag) {
    ...
    if (rand < pagMutationProb) {
        var its = getPageItems(pag);

        for (var i = 0; i < its.length; i++) {
            var it = its[i];

            if (it.constructor.name !== "Image" && randomFloat(0, 1) < itemMutationProb) {
                //logToFile("mutar item " + it.name);
                selectedItem = it;
        ...
        //Corre os mandatory_tools num item aleatorio
        var mand = interfaceSettings.mandatory_tools;
        for (var i = 0; i < mand.length; i++) {
            //alert(mand[i])
            //TODO: CHECK THIS WORKS
            selectedItem = its.randomItem();
            pageItemMethods.methods[mand[i].label].run({});
        }

    }
}

function mutateStrongly(pag) {
    var rand = randomFloat(0, 1);
    var mandatoryTools = interfaceSettings.mandatory_tools;
    ...
    if (rand < pagMutationProb) {
        ...
        for (var i = 0; i < its.length; i++) {
            ...
            if (it.constructor.name !== "Image" && randomFloat(0, 1) < itemMutationProb) {
                ...
                //mutate igual ao oneClickStylise:
                ...
                for (var k in methods) {
                    ...
                    if (Math.random() < Math.pow(m.probToRun, exp) || isMandatory) {
                        m.run({});
                    }
                }
            }
...
function createInitialPop(sel_basePag_nums) {
    ...
    var numSelectedPages = sel_basePag_nums.length; //numero de pags a partir das quais construir a init pop
    ...
    $.writeln("//1 — COPY SELECTED PAGES")
    //1 — COPY SELECTED PAGES (+add missing mandatory items)
    for (var i = 0; i < sel_basePag_nums.length; i++) {
        ...
        for (var j = its.length - 1; j >= 0; j--) {
            ...
            //TODO: substituir isto por array.removeItem(item)
            ...
            if (its[j] instanceof TextFrame) solveTextOverflow(it[j]); //TODO: expandTextBox(it)
        }
        ...
        for (var j = missingMandatoriyItems_names.length - 1; j >= 0; j--) {
            ...
            if (missingItem) {
                ...
                //remover text overflow
                if (mi instanceof TextFrame) solveTextOverflow(mi); //TODO: expandTextBox(it)
        ...
        if (colsCount < 2 && rowsCount < 2) { //se o user nao definiu //TODO: separar criação de cols e rows
    ...
    $.writeln("//2 - CREATE REMAINING PAGES BASED ON PREVIOUS ONES")
    //2 - CREATE REMAINING PAGES BASED ON PREVIOUS ONES
    var basePages_nums = [].concat(pop_pagNums);
    if (pop_size < numSelectedPages) {
        pop_size = numSelectedPages;
    ...
    for (var i = 0; i < numToCreate; i++) {
        //descomentar se quiser usar crossover
        /*var possiblePagNums = [].concat(basePages_nums);
        var child, p1 = page(possiblePagNums.spliceRandom());
        //CROSSOVER (nao vou usar para ser mais rapido)
        ...
        //comentar estas duas linhas se quiser usar crossover
        var p1 = page(basePages_nums.randomItem());
        var child = duplicatePage(p1);

        //MUTATION
        //mutate(child);
        mutateStrongly(child);

        //remover text overflow
        /*var its = child.pageItems;
        for (var j = 0; j < its.length; j++) {
            if (its[j] instanceof TextFrame) solveTextOverflow(its[j]); //TODO: expandTextBox(it)
        }*/
...
function stopEvolution(sorted_fitness) {
    $.writeln("stoping avolution in JSX")
    warnings(true);
    ...
    for (var i = sorted_fitness.length - 1; i >= 0; i--) {
        ...
        var fvalue = "g: " + sorted_fitness[i].fitness + " n: " + sorted_fitness[i].novelty + " b: " + sorted_fitness[i].balance + " l: " + sorted_fitness[i].legibility + "";
        text(fvalue, width + 10, 0, 300, 100);
...
function newPop(parents) {
    $.writeln("Variation: " + interfaceSettings.variation);
    if (interfaceSettings.variation == "mutation") newPopMutationOnly(parents);
    else newPopCrossoverAndMutation(parents);
}

//MUTATION AND CROSSOVER
function newChildCrossoverAndMutation(parent1Pag, parent2Pag) {
    //fileLog += "\nNew child";
...
function newPopCrossoverAndMutation(parents) {
    //parents são escolhidos em main.js 

    //$.gc();

    //elite
    $.writeln("-----------New pop\nCreating elite...")
    var elite_size = parents.elite.length;
    ...
    //parent pairs
    $.writeln("-----------Creating children...");
    for (var i = 0; i < parents.crossoverPairs.length; i++) {
        logToFile("-----------child number " + i);
        var ps = parents.crossoverPairs[i];
        var p = newChildCrossoverAndMutation(ps[0], ps[1]);
    }

    //alert("about to delete")
    $.writeln("-----------Deleting old pop... 1st:" + fst_popPagNum);
    $.writeln("-----------pop_pagNums.length: " + pop_pagNums.length);
    deleteOldPop();

    $.writeln("-----------new pop done");
    return JSON.stringify("-----------New pop has been created");
}

//MUTATION ONLY
function newChildMutationOnly(parentPag) {
    var parent = page(parentPag);
    var child = duplicatePage(parent); //todo: ver se isto esta a funcoinar bem
    mutate(child);
    //sentTextToTop(child);
    return child;
}

function newPopMutationOnly(parents) {
    //$.gc();
    ...
    //parent pairs
    $.writeln("-----------Creating children...");
    for (var i = 0; i < parents.selectedIndividuals.length; i++) {
        logToFile("-----------child number " + i);
        var ps = parents.selectedIndividuals[i];
        var p = newChildMutationOnly(ps);
    }
