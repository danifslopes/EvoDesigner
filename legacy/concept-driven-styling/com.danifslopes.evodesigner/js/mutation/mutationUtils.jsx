...
function getSubMethod(methDomain, methName, opts) {
    ...
    } else randomSubMethod = rouletteFromProbToRun(subMethods);
...
function initSwatches() {
    ...
    //create colors
    var numNewColors = semiConceptualRandomBetween(1, 15);
    for (var i = 0; i < numNewColors; i++) docMethods.methods.createColor.run();
    var numNewGradients = semiConceptualRandomBetween(1, 15);
    for (var i = 0; i < numNewGradients; i++) docMethods.methods.createGradient.run();

    deleteUnusedColorGroups();

    $.writeln("initSwatches toke " + (millis() - start) + " ms to load.");
}
...
function tryMutation(func) {
    ...
    if (err) {
        logToFile("Method canceled on " + selectedItem.name + " " + selectedItem.constructor.name + " Err: " + err)
...
function getPageItems(getOriginalObj) {
    var selectedPag = app.activeWindow.activePage || page();
    page(selectedPag);
