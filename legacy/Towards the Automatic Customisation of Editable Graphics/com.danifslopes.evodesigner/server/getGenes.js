...
async function getPropertiesFrom(pageID) {
    ...
    let content = await iframe.evaluate(async (pageID) => {
        ...
        function strIncludesAnyTheseWords(str, arrStrs) {
            let does = false;
            for (let s of arrStrs) {
                if (str.indexOf(s) > -1) {
                    does = true;
                    break;
                }
        ...
        async function tableRowToJSON(tableRow, headers) {
            ...
            //Ignorar readonly
            if (columns[2].indexOf("readonly") > -1) return false;
            ...
            for (let i in columns) {
                ...
                if (i == 1) {
                    ...
                    if (strIncludesAnyTheseWords(lcType, ["event"]) || ["string", "object", "function"].includes(lcType)) return false;

                    //determinar tipos que ja conhecemos 
                    if (lcType == "boolean") {
                        possibleValues.constants = [{
                            value: true
                        }, {
                            value: false
                        }];

                    } else if (["number", "numbers", "real", "reals", "unit", "units"].includes(lcType)) {
                ...
                //inherited or overridden value
                let useNeg1Index = s.indexOf("use -1");
                if (useNeg1Index > -1) possibleValues.inherited = -1;
                ...
                //colors
                if (columns[1].toLowerCase().indexOf("color") > -1) {
                    columns[1] = "Color";
                    possibleValues.arrayLength = 3;
                    possibleValues.from = 0;
                    possibleValues.to = 255;
                }
                ...
                if (i == 3) {
                    ...
                    let toIgnore = ["number", "numbers", "real", "reals", "unit", "units", "boolean", "booleans", "string", "number range"];
                    toIgnore.push(pageID.toLowerCase());
                    if (!toIgnore.includes(type.toLowerCase())) {
                        //Pesquisa dentro
        ...
        async function valuesTableToPossibleValues(table) {
            ...
            for (let i = 1; i < table.rows.length; i++) {
                ...
                possibleValues.constants.push(valueJSON);
            }
        ...
        async function tableToJson(table) {
            ...
            let isValues = headers == ["Name", "Description", "Value"].toString();

            if (isProperties) {
                for (let i = 1; i < table.rows.length; i++) {
                    let row = table.rows[i];
                    let rowJSON = await tableRowToJSON(row, headers);
                    if (rowJSON) data[rowJSON.Type] = rowJSON;
                }
